...
var Dangnhap = () => {
  return (
    <ScrollView>
      <Text style={{padding:20,backgroundColor:'#e8ecf3'}} onPress={()=> Actions.pop()}>
        <Icon name="times" size={30} color="#00a888"/>
      </Text>
      <Login />
      <Text style={{color:'#00a888', fontSize:18, paddingTop:20,textAlign:'center'}}
       onPress={() => Actions.register()}>Bạn quên mật khẩu?</Text>
      <View style={{flexDirection:'row', justifyContent:'center',marginBottom:60}}>
        ...
        <Text style={{color:'#00a888', fontSize:18, paddingTop:20}} onPress={() => Actions.register()}> Đăng ký</Text>
      </View>
    </ScrollView>
  );
}
...
class Login extends Component{
  ...
  render(){
    return(
      ...
      <Akira
      label={'Tên đăng nhập hoặc Email'} style={styles.input}
